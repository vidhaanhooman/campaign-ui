...
const CARD =
  "rounded-lg border bg-surface shadow-xl shadow-black/40";
    ...
    >
      ...
      <PopoverTrigger
        render={
          <button
            type="button"
            className="flex h-9 w-full items-center gap-2.5 rounded-md border border-border-strong bg-surface-2 px-3 text-left text-sm text-text outline-none transition-colors hover:border-text-muted/40 focus:border-white"
          />
        }
      >
        {agent ? (
      ...
      <PopoverContent
        ...
        sideOffset={6}
        style={SURFACE_BG}
        className={cn(CARD, "p-0 w-[720px] max-w-[94vw] overflow-hidden border-[color:var(--border-strong)]")}
      >
        <div className="grid h-[400px] grid-cols-[1fr_280px] grid-rows-[auto_1fr_auto]">
          {/* HEADERS */}
          <div className="flex h-12 items-center gap-2 border-b border-r border-border px-4 text-sm font-medium text-text">
                      ...
                      <span className="min-w-0 flex-1">
                        <span
                          className={cn(
                            ...
                            checked ? "font-medium text-text" : "text-text-dim",
                          )}
                        >
                          {a.name}
                        </span>
          ...
          <div className="flex min-h-0 flex-col overflow-hidden">
            {!draftAgentDetail ? (
              <div className="flex flex-1 items-center justify-center px-5 text-center text-xs leading-relaxed text-text-muted">
          ...
          >
            ...
            <button
              onClick={reset}
              className="rounded-md border border-border-strong px-3 py-1.5 text-xs text-text-dim transition-colors hover:text-text"
            >
              Clear
            </button>
