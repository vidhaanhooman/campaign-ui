...
export function EmptyState({
    ...
    return (
        <div
            className={cn(
                "shadcn dark flex flex-col items-center justify-center text-center",
...
export function EmptyAction({ onClick, children }: { onClick?: () => void; children: React.ReactNode }) {
    return (
        <button
            type="button"
            onClick={onClick}
            className="shadcn dark inline-flex h-8 items-center gap-1.5 rounded-lg border border-border bg-card px-3 text-xs font-medium text-muted-foreground transition-colors hover:border-ring hover:text-foreground"
        >
            {children}
        </button>
    );
