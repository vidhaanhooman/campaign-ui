...
/** Titled surface used for each deep-dive sub-panel. */
function DeepCard({
  title,
  className,
  children,
...
function DurationFunnel({ stages }: { stages: FunnelStage[] }) {
  ...
  return (
    <div className="space-y-4">
        ...
        {stages.map((s, i) => {
          ...
          return (
            <div key={s.stage} className="flex justify-center">
              <div
                className="flex h-11 items-center justify-center rounded-md text-sm font-medium text-white shadow-sm"
                style={{
                  width: `${width}%`,
                  background: `color-mix(in srgb, var(--chart-2) ${100 - i * 13}%, #11151f)`,
                }}
              >
                {s.value}
              </div>
      ...
      <div className="space-y-1 border-t border-white/[0.06] pt-3 text-xs text-muted-foreground">
        {stages.map((s, i) => {
          const prev = i === 0 ? s.value : stages[i - 1].value;
          const cont = prev ? Math.round((s.value / prev) * 100) : 0;
          return (
            <p key={s.stage}>
              <span className="text-foreground">{s.stage}</span>: {s.value} ({s.pctOfFirst}% of first)
...
function CallsDeepDive({
  ...
  const tiles = [
    { label: "Connected", value: connected.toLocaleString(), sub: "" },
    ...
    { label: "Agents", value: String(agents.data.length), sub: "" },
  ];
      ...
      <div className="grid gap-3.5 lg:grid-cols-2">
        <DeepCard title="Duration wise calls funnel">
          {funnel.loading ? (
            ...
            <DurationFunnel stages={funnel.data} />
          )}
        </DeepCard>
  ...
  if (metric === "pickup") {
    return (
      <div className="h-full overflow-y-auto pr-1">
        <MetricDetail label={label} range={range} refreshKey={refreshKey} full />
      </div>
    );
  }
