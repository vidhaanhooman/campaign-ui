 ...
 *  - In-dropdown hover:     bg-white/[0.04]
 *  - In-dropdown chip/search: border-white/[0.06] bg-transparent
 *  - Primary/accent:      bg-primary text-primary-foreground   (white on black)
 *  - Radii:               rounded-lg controls · rounded-xl cards · rounded-md chips
 *
 * Drop this file in, feed it a schema, and use it.
 */
...
const CARD =
  "rounded-xl border border-border bg-[#090909]";
const CONTROL =
...
export function FilterDropdown({
  ...
  const visibleSections = useMemo(() => {
    ...
    return schema
      .map((s) => ({
        ...s,
        items: s.items.filter((c) => c.label.toLowerCase().includes(query)),
      }))
  ...
  const activeCat =
    active === "__quick__"
      ? null
      : (allCats.find((c) => c.id === active) ?? null);

  const setOne = (id: string, v: FilterValue | null) =>
    onChange({ ...value, [id]: v });
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        ...
        className={cn(
          CARD,
          "flex max-h-[80vh] flex-col overflow-hidden p-0 shadow-xl",
        )}
      >
        {/* Search */}
        <div className="border-b border-white/[0.04] p-2">
          <div
            className={cn(
              "flex h-8 items-center gap-2 rounded-lg border border-white/[0.06] bg-transparent px-2.5",
            )}
          >
          ...
          {quickFilters && quickFilters.length > 0 && !query && (
            <>
              <Row
                ...
                onClick={() =>
                  setActive(active === "__quick__" ? null : "__quick__")
                }
              />
              {active === "__quick__" && (
                ...
                return (
                  <div key={cat.id}>
                    <Row
                      ...
                      disabled={isDisabled}
                      disabledReason={
                        disabledResult ? disabledResult.reason : undefined
                      }
                      isActive={isOpen}
                      onClick={() =>
                        !isDisabled && setActive(isOpen ? null : cat.id)
                      }
                    />
                    {isOpen && (
...
function EditorForCategory({
  ...
  switch (cat.type) {
    ...
    case "range":
      return (
        <RangeEditor
          cat={cat}
          value={
            (value as NumberRange) ?? { kind: "range", min: null, max: null }
          }
          onChange={onChange}
        />
      );
    ...
    case "text":
      return (
        <TextEditor cat={cat} value={(value as string) ?? ""} onChange={onChange} />
      );
    case "custom":
...
// ---- Range slider (native + inputs — matches shadcn slider look) ----------
...
/* ===========================================================================
 * EXAMPLE — delete or adapt for your domain
 * ---------------------------------------------------------------------------
 * import { Bot, Flag, Hash, Phone, Tag, Timer } from "lucide-react";
 *
 ...
 *         searchable: true,
 *         options: [
 *           { value: "a1", label: "Agent A" },
 *           { value: "a2", label: "Agent B" },
 *         ],
 *       },
 *     ],
 *   },
