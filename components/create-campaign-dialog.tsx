  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        ...
        className="!max-w-[960px] p-0 overflow-hidden flex flex-col border-l border-border shadow-2xl shadow-black/60"
      >
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <ArrowRight size={14} className="text-muted-foreground transition-transform group-hover:translate-x-0.5 group-hover:text-foreground" />
                  </div>
                  ...
                  <div className="mt-auto space-y-3 pt-3 border-t border-border">
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-medium text-foreground">{opt.needs}</span>
                    </div>
                  </div>
                </button>
              );
    ...
    <div className="rounded-xl border border-border bg-[#333333]/30 overflow-hidden">
      ...
      <div className="p-4 space-y-3">
        ...
        <SummaryRow label="API overrides">
          {overrides.size === 0 ? (
            <span className="text-muted-foreground italic">All locked</span>
          ) : (
      ...
      <>
        ...
        {inner.map((part, i) => (
          <span key={i}>
            ...
            {i < inner.length - 1 && <span className="text-muted-foreground">, </span>}
          </span>
        ))}
        <span className="text-muted-foreground">]</span>
      </>
...
function PayloadCard({
  ...
  return (
    <div className="rounded-xl border border-border bg-[#333333]/30 p-4 space-y-4">
      ...
      <pre className="rounded-md border border-border p-3 font-mono text-xs leading-[1.8] whitespace-pre-wrap text-foreground" style={{ backgroundColor: "var(--background)" }}>
        <span className="text-muted-foreground">{"{"}</span>
        {required.map(([k, v], i) => (
          <div key={i} className="pl-4">
            ...
            <JsonValue value={v} />
            <span className="text-muted-foreground">,</span>
            <span className="ml-2 text-muted-foreground/60">{`// required`}</span>
          </div>
        ))}
        {optionalLines.map(([k, v], i) => (
          <div key={i} className="pl-4">
            ...
            <span className="ml-2 text-muted-foreground/60">{`// override`}</span>
          </div>
        ))}
        <span className="text-muted-foreground">{"}"}</span>
      </pre>
